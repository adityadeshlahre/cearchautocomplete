...
cleanup() {
  ...
  if docker ps -a --filter "name=cearchautocomplete-full" --format '{{.Names}}' | grep -q '^cearchautocomplete-full$'; then
    echo "Stopping and removing application container..."
    docker stop cearchautocomplete-full
    docker rm cearchautocomplete-full
  fi
